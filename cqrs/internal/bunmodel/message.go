...
type Message struct {
	...
	ExternalID string `bun:"external_id,notnull" yaml:"external_id"`
	// SenderID is the foreign key referencing the sending user.
	SenderID int64 `bun:"sender_id,notnull" yaml:"sender_id"`
	// ReceiverID is the foreign key referencing the receiving user.
	ReceiverID int64 `bun:"receiver_id,notnull" yaml:"receiver_id"`
	// Subject is the message subject line.
	Subject string `bun:"subject" yaml:"subject"`
	// Text is the message body.
	Text string `bun:"text" yaml:"text"`
	// DateSent is the UTC timestamp when the message was sent.
	DateSent time.Time `bun:"date_sent" yaml:"date_sent"`
